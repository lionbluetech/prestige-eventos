...
export default function CantidadesPage() {
  ...
  useEffect(() => {
    ...
    calcularCantidadesSupabase(data.evento, data.hombres, data.mujeres, data.consumo, data.tragos)
      .then((r) => {
        setResultado(r);
        // Inicializar pesos iguales para todos los tragos
  ...
  // Recalcular servings según los pesos actuales
  ...
  return (
    <main className="min-h-screen bg-black text-white flex flex-col px-6 py-3 gap-2">

      {/* HEADER */}
      <div className="text-center shrink-0">
        <h1 className="text-2xl font-bold">PLANIFICÁ TU <span className="text-yellow-500">BARRA IDEAL</span></h1>
        <p className="text-zinc-400 text-xs">Todo lo que necesitás para un evento perfecto</p>
      </div>
      ...
      <div className="flex justify-center items-center gap-6 shrink-0">
        {["EVENTO", "PERSONAS", "CONSUMO", "TRAGOS", "CANTIDADES"].map((paso, i) => (
          <div key={i} className="flex items-center gap-1.5">
            <div className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold ${i < 4 ? "bg-green-500 text-black" : "bg-yellow-500 text-black"}`}>
              {i < 4 ? "✓" : "5"}
            </div>
            <span className={`text-xs uppercase font-semibold ${i === 4 ? "text-yellow-500" : "text-zinc-400"}`}>{paso}</span>
          </div>
        ))}
      </div>

      {/* FILA 1: Evento + KPIs + Tragos con sliders */}
      <div className="grid grid-cols-[220px_1fr] gap-3 shrink-0">

        {/* Imagen evento */}
        <div className="relative rounded-2xl overflow-hidden border border-yellow-700/30 h-36">
          <Image src={getEventoImage()} alt={data.evento} fill className="object-cover" />
          <div className="absolute inset-0 bg-gradient-to-r from-black/90 via-black/60 to-transparent" />
          <div className="absolute inset-0 p-4 flex flex-col justify-between">
            <div>
              <p className="text-yellow-500 text-xs uppercase tracking-widest">Evento</p>
              <h2 className="text-lg font-bold">{data.evento}</h2>
            </div>
            <div className="space-y-0.5 text-xs">
              {[["Invitados", data.personas], ["Consumo", data.consumo], ["Tragos", data.tragos.length]].map(([l, v]) => (
        ...
        {/* KPIs + tragos con sliders */}
        <div className="flex flex-col gap-2">
          <div className="grid grid-cols-4 gap-2">
            {[
              ["TRAGOS TOTALES", resultado.totalTragos, "tragos estimados"],
              ["POR CÓCTEL", tragoDetalleActivo?.tragos || 0, "del trago activo"],
              ["HIELO NECESARIO", `${resultado.hieloKg} kg`, "total estimado"],
              ["DURACIÓN ESTIMADA", `${horas} hs`, "del evento"],
            ].map(([label, val, sub]) => (
              <div key={label as string} className="bg-zinc-950 border border-yellow-700/30 rounded-xl p-3 text-center">
          ...
          {/* Cards de tragos con slider */}
          <div className="bg-zinc-950 border border-yellow-700/30 rounded-xl px-3 py-2">
            <p className="text-yellow-500 text-xs uppercase tracking-wide mb-2">
              ¿¿Cuál es la preferencia de trago de tus invitados? Ajustá el slider
            </p>
              ...
              {detallePonderado.map((item, i) => {
                ...
                return (
                  <div
                    ...
                    className={`flex-shrink-0 flex flex-col items-center gap-1 rounded-xl border p-2 cursor-pointer transition-all w-24 ${tragoActivo === i ? "border-yellow-500 bg-yellow-500/10" : "border-zinc-700 hover:border-yellow-700"}`}
                  >
                    <div className="relative w-12 h-12 rounded-lg overflow-hidden bg-zinc-800">
                      {TRAGO_IMAGEN[item.trago]
                        ? <Image src={TRAGO_IMAGEN[item.trago]} alt={item.trago} fill className="object-cover" />
                        : <div className="w-full h-full flex items-center justify-center text-lg">🍹</div>}
                    </div>
                    <p className="text-xs text-center w-full truncate">{item.trago}</p>
                    <p className="text-xs text-yellow-500 font-bold">{item.tragos} tragos</p>
                    <div className="w-full flex items-center gap-1">
                      <input
                        ...
                        onChange={(e) => {
                          e.stopPropagation();
                          setPesos((prev) => ({ ...prev, [item.trago]: Number(e.target.value) }));
                        }}
      ...
      {/* FILA 2: 4 paneles */}
      <div className="grid grid-cols-[1fr_1fr_1fr_240px] gap-3 flex-1">
              ...
              <div className="flex items-center gap-3 mb-3 pb-2 border-b border-yellow-500/20">
                <div className="relative w-8 h-8 rounded-lg overflow-hidden bg-zinc-800 flex-shrink-0">
                  {TRAGO_IMAGEN[tragoDetalleActivo.trago]
                    ? <Image src={TRAGO_IMAGEN[tragoDetalleActivo.trago]} alt={tragoDetalleActivo.trago} fill className="object-cover" />
        ...
        <div className="bg-zinc-950 border border-yellow-700/30 rounded-2xl p-4 flex flex-col">
          ...
          {ingredientesTotales.length > 4 && (
            <button
              onClick={() => setVerTodosIng(!verTodosIng)}
              className="mt-2 text-yellow-500 text-xs hover:text-yellow-400 transition"
            >
              {verTodosIng ? "Ver menos ↑" : `Ver todos los ingredientes (${ingredientesTotales.length}) ↓`}
            </button>
          )}
        </div>
